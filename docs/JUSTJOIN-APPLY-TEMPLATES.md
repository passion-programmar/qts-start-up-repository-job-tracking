...
Auto-apply and manual confirm use a **two-phase** flow:

| Phase | When | What fills |
|-------|------|------------|
| **1 — Profile** | Right after template detect + modal open | **Name**, **Email** from selected candidate |
| **2 — Custom GPT** | After profile saved to server | **Resume PDF** generated by GPT → uploaded to “Add document” |

Never auto-filled: terms, GDPR, marketing, cover message toggle, final Submit.

| Field | Phase 1 (static) | Phase 2 (GPT) |
|-------|------------------|---------------|
| `name` | Yes | — |
| `email` | Yes | — |
| `resume` | — | Yes (PDF upload) |
| `terms`, `gdpr` | No — manual | — |
| `marketing` | No — manual | — |
| `cover_toggle` / message | No — manual | — |

Fill policy flags: `fillProfileBeforeGpt`, `gptApplyDocumentsOnly`, `deferDocumentUploadToGpt`.
